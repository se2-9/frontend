  ...
  return (
    <MaxWidthWrapper className="py-8">
      <div className="flex flex-col items-center">
        <div className={`w-full ${hasCards ? 'mb-8' : 'h-[calc(100vh-160px)] flex flex-col justify-center'}`}>
          <Dialog open={showAddCardDialog} onOpenChange={setShowAddCardDialog}>
            ...
            <DialogContent className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" aria-description="Add your card">
        ...
        {hasCards && (
          <div className="w-full space-y-6">
            {allCards.map((card: CardDTO) => (
              <label key={card.id} className="relative flex cursor-pointer rounded-lg border p-1 m-1 items-center space-x-2">
                <div className="w-full">
                  ...
                  >
                    ...
                    <div className="mb-6">
                      <div className="text-lg font-mono tracking-wider">•••• •••• •••• {card.number.slice(-4)}</div>
                    </div>
                    <div className="flex justify-between items-end">
                      <div>
                        <span className="text-xs uppercase opacity-80">Expires</span>
                        <div className="font-medium">
                          {card.expiration_month.toString().padStart(2, '0')}/{card.expiration_year.toString().slice(-2)}
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <Button className="bg-app-lightbrown text-lightbrown-foreground hover:bg-red-400" onClick={()=>{handleDeleteCard(card.number)}}>
